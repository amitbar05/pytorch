"""T.10 / CP.3 — RNN / LSTM / GRU compile coverage (forward + backward).

CP.3 / OP.6 (2026-05-09): Replaced the CPU-roundtrip in the eager intercept
with a Vulkan-native Slang template (``templates/rnn_cell.py.jinja``).
The old CPU-roundtrip custom ops are retained as fallback (gated by
``TORCH_VULKAN_RNN_CPU_FALLBACK=1``) while the template path matures.

Three pieces are wired up:

1. **Internal custom op (T.10-bwd).** Each RNN family gets a
   ``torch.library.custom_op`` named ``torch_vulkan::<vf>_cpu_roundtrip``
   that routes through CPU for backward compatibility.

2. **Eager-side intercept (CP.3 — template path).** ``aten.lstm.input`` /
   ``aten.gru.input`` / ``aten.rnn_*`` are intercepted at
   ``AutogradPrivateUse1``. The default path now loops over time steps
   and dispatches each cell through the ``rnn_cell.py.jinja`` Slang
   template — no CPU transfer. Set ``TORCH_VULKAN_RNN_CPU_FALLBACK=1``
   to restore the old CPU-roundtrip path.

3. **Compile-side fallback.** Inductor ``make_fallback`` emits an
   ``ExternKernel`` that dispatches through the eager path at runtime.
"""
...
def _register_rnn_eager_intercepts() -> None:
    ...
    _RNN_LIB.impl("lstm.input", _lstm_input_impl, "AutogradPrivateUse1")
    _RNN_LIB.impl("lstm.data", _lstm_data_impl, "AutogradPrivateUse1")
    _RNN_LIB.impl("gru.input", _gru_input_impl, "AutogradPrivateUse1")
    _RNN_LIB.impl("gru.data", _gru_data_impl, "AutogradPrivateUse1")
    _RNN_LIB.impl("rnn_tanh.input", _rnn_tanh_input_impl, "AutogradPrivateUse1")
    _RNN_LIB.impl("rnn_tanh.data", _rnn_tanh_data_impl, "AutogradPrivateUse1")
    _RNN_LIB.impl("rnn_relu.input", _rnn_relu_input_impl, "AutogradPrivateUse1")
    _RNN_LIB.impl("rnn_relu.data", _rnn_relu_data_impl, "AutogradPrivateUse1")
