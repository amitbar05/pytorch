...
class PointwiseEntry:
    ...
    `complex_valued`: if True, the op operates on `float2` (complex)
    instead of `float` (real).  The Slang struct must implement
    `IComplexPointwise` / `IComplexPointwiseBinary` rather than
    `IPointwise` / `IPointwiseBinary`.
    """
...
# ── Complex-valued pointwise dispatch (OP.20) ───────────────────────────
# Complex ops use float2 SSBO layouts and the IComplexPointwise /
# IComplexPointwiseBinary interfaces in pointwise.slang.  This table
# mirrors POINTWISE_TABLE but keys complex-valued aten ops — same aten
# op name, different Slang struct and buffer type.  Callers must consult
# this table FIRST when input tensors have complex dtype.
COMPLEX_POINTWISE_TABLE: dict[str, PointwiseEntry] = {
    # Binary complex (float2 × float2 → float2)
    "aten.add": PointwiseEntry("OpComplexAdd", 2, complex_valued=True),
    "aten.sub": PointwiseEntry("OpComplexSub", 2, complex_valued=True),
    "aten.mul": PointwiseEntry("OpComplexMul", 2, complex_valued=True),
    "aten.div": PointwiseEntry("OpComplexDiv", 2, complex_valued=True),
    # Unary complex (float2 → float2)
    "aten.conj_physical": PointwiseEntry("OpComplexConj", 1, complex_valued=True),
    # Unary complex-to-real: |x| returns float (packed into float2.x).
    "aten.abs": PointwiseEntry("OpComplexAbs", 1, complex_valued=True),
}
