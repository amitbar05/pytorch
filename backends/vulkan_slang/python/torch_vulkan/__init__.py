...
def _register_optional_tensor_workarounds():
    ...
    # Use a shared flag on the torch module so inductor._legacy_register()
    # can set it after calling register_eager_patch_custom_ops(), preventing
    # Dynamo SpeculationLogDivergence on retrace (2026-05-28 fix).
    if not hasattr(torch, "_torch_vulkan_patch_custom_ops_done"):
        torch._torch_vulkan_patch_custom_ops_done = False
    ...
    def _ensure_patch_custom_ops():
        ...
        if torch._torch_vulkan_patch_custom_ops_done:
            return
    ...
    def _patched_batch_norm(
        ...
        if _is_vulkan(input):
            ...
            if training:
                ...
                # mean: reduce over N, H, W dims → shape [C]
                mean_val = input.mean(dim=reduce_dims, keepdim=False)
                # var: E[(x - mean)^2]
                bcast = [1, -1] + [1] * (input.dim() - 2)
                mean_b = mean_val.reshape(bcast)
                input_centered = input - mean_b
                var_val = (input_centered * input_centered).mean(
                    dim=reduce_dims, keepdim=False
                )
                
                # Normalize
                invstd = 1.0 / (var_val + eps).sqrt()
                invstd_b = invstd.reshape(bcast)
                weight_b = weight.reshape(bcast)
                bias_b = bias.reshape(bcast)
                ...
                running_mean.copy_(
                    (1 - momentum) * running_mean + momentum * mean_val.detach()
                )
                running_var.copy_(
                    (1 - momentum) * running_var
                    + momentum * var_val.detach() * (n / (n - 1))
                )
                return out
