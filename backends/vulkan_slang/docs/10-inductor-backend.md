...
| # | Pillar | Title | Effort | Status |
|---|--------|-------|--------|--------|
| **DISP.1** | V13-DISP | GN forward fusion: ExternKernelOut (1 dispatch vs ~10) | 0.5 d | ✅ **CLOSED 2026-06-02.** Always-on. ``_register_group_norm_fused()`` now called unconditionally. Fused GroupNorm forward via ``group_norm.slang`` replaces ~10 dispatch decomposition with 1 fused dispatch. Nested-StorageBox fix applied to ``_vk_realize_then_unwrap`` in ``gn_forward_extern.py``. |\n| **DISP.2** | V13-DISP | BN forward fusion: ExternKernelOut (1 dispatch vs ~12) | 0.5 d | 🔲 OPEN — Same pattern as GN.1 but for BatchNorm forward. |\n| **DISP.3** | V13-DISP | Conv+GN+ReLU combo fusion (33→10 dispatches) | 1 d | 🟡 **PARTIAL 2026-06-02.** Forward fusion works via ``install_conv_patched_gn_relu_fusion()`` — produces ``conv2d_gn_relu_fused`` nodes. Forward wrapper: 10 dispatches (down from 33). Backward crashes: ExternKernelSchedulerNode receives nested StorageBox→Pointwise inputs. Nested-StorageBox fix + lowering-level pre-realization attempted but Pointwise still escapes. Combo fusion NOT active in production. |\n| **VRFY.1** | V13-VRFY | Flip BN.2 xfail → PASS on RDNA1 | 0.25 d | 🔲 OPEN — TestV10BN2_BatchNormCompileTraining currently xfail(strict=True). Root cause (helpers capability) fixed 2026-06-01. |
