...
| **M18.HS.1** | `F.hardsigmoid` decomposes to `clamp` — FakeTensorProp data-access error | ❌ **NEW 2026-05-23** | `F.hardsigmoid` in the compile-mode FX graph decomposes to `clamp(x*(1/6)+0.5, 0, 1)`. The Inductor codegen hits `torch.ops.aten.clamp` during `fake_tensor_prop`, which tries to access the data pointer of a FakeTensor → `InductorError: Cannot access data pointer of Tensor`. Fix: suppress the hardsigmoid decomposition in Vulkan's `meta_patches/decomposition_passes.py` (route to native `hardsigmoid` lowering) OR add a composite Vulkan lowering that intercepts the clamp-form. See `test_hardsigmoid_backward_grad_parity` (xfail strict=False). |
...
| **TEST.COV.3** | ✅ **DONE 2026-05-23** — `TestTestCov3ActivationBwd` (24 tests) added for hardtanh/hardsigmoid/softplus/mish + 6 basic trig (sin/cos/tan/asin/acos/atan) + 6 hyperbolic (sinh/cosh/tanh/asinh/acosh/atanh) + 8 exp/log (exp2/expm1/log2/log10/log1p/erf/erfc/sqrt). All dispatch through `bwd_diff_table.py` → `bwd_diff(<op>_fwd)` → pointwise backward shaders, except `softplus` (algebraic route via M-AG5.1) and `hardtanh` (via clamp_min+clamp_max decomp per M18.TB.1). Note: `torch.special.cbrt` not available in PyTorch 2.11; excluded. **1 test xfail (strict=False):** `test_hardsigmoid` (M18.HS.1: hardsigmoid decomposes to clamp → FakeTensorProp data-access error). `test_hardtanh` now PASSES (xfail removed per M18.TB.1 DONE). 22 remaining tests require warm SPIR-V cache due to FA prewarm thread-pool blocking on cold cache. |
