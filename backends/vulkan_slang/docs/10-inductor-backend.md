...
| # | Title | Status |
|---|-------|--------|
| **M22.1.a-g** | Split file-size violators | `conv.py` ✅ M22b; `rnn.py` ✅ M22d (700 L); `wrapper.py` ✅ M22.1.h (642 L + `wrapper_helpers.py` 254 L); `scheduling.py` ✅ M22c (781 L + `scheduling_helpers.py` 134 L); `runtime/slangc.py` M22a Stage 1 ✅ **DONE 2026-05-22** — 2342 → 2030 L, new `common.py` 374 L (slangc binary resolver, source normalizer, device tag, async flags, caches, locks, stats, pool guard). Stages 2–7 still needed (target ≤ 800 L). Commit `975748a916e`. |
