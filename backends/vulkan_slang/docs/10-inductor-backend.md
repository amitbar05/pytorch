...
| **M-AUDIT-PERF.1-followup** | Dim-reduced sum/mean backward gradient = wrong values (100 % mismatch vs CPU, gradients ~4× off) | ✅ FIXED 2026-05-20 | `_rewrite_constant_folded_tangent` in `joint_graph_passes.py:448-686` extended to handle non-scalar tangent shapes via greedy right-to-left `_compute_view_shape` dim-matcher (see § 0.0.9 row 1). **Verified 2026-05-24**: `test_cgm3_sum_dim_backward_matches_cpu` PASS (198s compile), `test_cgm3_mean_dim_backward_matches_cpu` also PASS. All 14 cgm3 backward tests pass. |
...
| **M-CV.2** | 30 zero-coverage backward ops identified | ✅ **Phase 1 DONE 2026-05-24** (cos/log2/atan2/maximum/minimum + sin/sqrt/erf/log/abs, 8 tests, commit `4c894c7ce37`). ✅ **Phase 2 DONE 2026-05-24** (pow.Tensor_Tensor/hypot/reciprocal, 8 tests, commit `f30f23e4c19`). Root-cause fix: `_where_self_fake` lacked broadcasting — `where(cond, scalar_0d, tensor_N)` returned `[]` instead of `[N]`. |
...
| # | Title | Status |
|---|-------|--------|
| **M20.1** | RNN cell backward via autodiff (238 L hand-rolled → bwd_diff) | ⚠️ **PARTIAL 2026-05-25** — Slang template side done: `templates/rnn_cell_bwd.slang` now delegates activation derivatives to `bwd_diff(sigmoid_fwd)` / `bwd_diff(tanh_fwd)` / `bwd_diff(relu_fwd)` (lib/pointwise.slang) instead of inline `_dsigmoid_from_val` / `_dtanh_from_val` helpers. Tests: `TestM201RNNCellAutodiff` (3/3 pass: bwd_diff present, slangc compiles all 4 cell types, no inline derivative helpers). Python-side `lowerings/rnn_bwd.py` (~398 L) cannot be replaced: (a) it handles `aten._thnn_differentiable_lstm_cell_backward` / `_thnn_fused_lstm_cell_backward_impl` / `_thnn_differentiable_gru_cell_backward` as PrivateUse1 safety-net fallbacks — these ops have different signatures (no weight tensors) from the Slang template path (OP.25 BPTT); (b) full Slang cell autodiff (`[Differentiable]` over the entire cell update) is blocked because `slang_validate/bwd_diff_scan.py` requires every `[Differentiable]`-tagged function to have a paired `[BackwardDerivative]` — `bwd_diff()` call-sites alone don't satisfy this. Follow-on: M20.1.e — patch the validator to recognize `bwd_diff()` use sites; M20.1.f — add `[Differentiable]` to `rnn_cell.slang` cell update and replace the 3 Python helper functions with a Slang-native autodiff call. Also fixed in same session: `TestT10BwdRnnBackwardCompile` (5 tests) were failing with `Dynamo does not support RNN/GRU/LSTM` — added `torch._dynamo.config.allow_rnn = True` to each test method. |
...
| # | Title | Status |
|---|-------|--------|
| **M22.1.a-g** | Split file-size violators | `conv.py` ✅ M22b; `rnn.py` ✅ M22d (700 L); `wrapper.py` ✅ M22.1.h (642 L + `wrapper_helpers.py` 254 L); `scheduling.py` ✅ M22c (781 L + `scheduling_helpers.py` 134 L); `runtime/slangc.py` ✅ **DONE 2026-05-24** — verified at 781 L (< 800 L cap) via `common.py` (374 L) + `shader_lib.py` (776 L) + `reflection_ext.py` extractions. Commits `975748a916e`, `78bd544240d`. |
...
| **M22.2** | `alloc_alias.py` IR-level migration (280 L regex post-processor) | ✅ **DONE 2026-05-24** — dead code (`_alloc_alias_fn`, `_get_alloc_alias_fn`) removed from `wrapper_helpers.py`; `alloc_alias.py` docstring marked `.. deprecated:: M22.2`; IR-level pass (`alloc_alias_ir.apply_vulkan_ir_alias_pass`) was already fully wired into `wrapper.py::run_wrapper_ir_passes()`. Tests: `TestM222AllocAliasIRMigration` 10/10. Commit `62dee86`. |
| **M22.3** | Pre-grad pattern firing-rate instrumentation | ✅ **DONE 2026-05-24** — `_PatternStats`, `record_pattern_fire`, `dump_pattern_stats`, `reset_pattern_stats` in `post_grad.py`; wired into `FxPatternEntry.apply()` in `patterns/registry.py:69-77` (guarded by `pattern_stats_enabled()`); `pattern_stats_enabled()` in `config.py:568`; `dump_pattern_stats()` called in `_VulkanCustomPass.__call__`. Tests: `TestM223PatternStats` at `test_inductor_regression.py:55674` (3 tests). |
...
| **M18.9-fused-bwd-mask** | **Fused `conv2d_gn_relu_fused` backward: ReLU mask drawn from Slang output but GN bwd uses aten-recomputed conv_out** | ✅ **FIXED 2026-05-27** | `_conv2d_gn_relu_setup_context` recomputes `conv_out` + GN stats via `aten.convolution` / `aten.native_group_norm` so the GN backward operates on aten-consistent data — but the saved ReLU mask was `(slang_output > 0)`, taken from the fused Slang kernel's post-ReLU output. Near-zero elements where the Slang Welford reduction order differs from aten flip the mask, mismatching the conv_out/mean/rstd triple that `native_group_norm_backward` operates on. Sum reduction in conv-bias bwd accumulates the per-element flips → `c.bias L_inf=0.885` (~10 % of CPU norm) failing `test_fused_conv_gn_relu_backward_grad_parity`. **Fix** (`fx_passes/eager/conv_gn_relu.py`): capture the aten-computed `gn_out` (was discarded as `_`) in setup_context and derive the ReLU mask from `(saved_gn_out > 0)` — functionally equivalent to `relu(gn_out) > 0`, numerically consistent with what the backward reduces against. Test PASS (parity now <1e-4). |
...
| # | Title | Severity | Detail |
|---|-------|----------|--------|
| **M-NEW.1** | ✅ **CLOSED 2026-05-27** | done | The duplicate-`ExternKernelChoice` name was already mitigated by `_EXTERN_CHOICE_CACHE` + upstream `lazy_register_extern_choice`. Remaining symptom was a 2nd-order **batch-executor re-entrant deadlock** in `runtime/slangc.py::batch_compile_slang_to_spirv`: workers inside the local pool called `compile_slang_to_spirv` which re-submitted to `_ASYNC_POOL`, but `_ASYNC_POOL`'s 2 default workers were busy on `_ensure_shader_lib_modules` (slangc lib-module precompile under a serial lock). Watchdog stack dump (`agent_space/probe_compile_hang_w2.py`) pinned the chain. **Fix**: wrap `_compile_one` with `_wrap_pool_worker` so the thread-local `in_pool_worker=True` flag flips `compile_slang_to_spirv`'s PF.14 bypass — batch workers compile inline instead of bouncing to `_ASYNC_POOL`. Tests: `TestMNew1DuplicateExternKernel` (4/4 PASS; 2-Linear E2E 0.28 s vs ∞ before). |
| **M-NEW.2** | ✅ **CLOSED 2026-05-27 (cascade resolved with M-NEW.1)** | done | Conv2d compile no longer segfaults — same Linear-backward extern path; M-NEW.1's deadlock fix removed the LoweringException unwind that was poisoning autotune registry. Probe `agent_space/probe_conv_compile.py` confirms compile completes (slow cold path on contended box is M-NEW.5 territory, not a crash). |
...
| **M-NEW.6** | ✅ **CLOSED 2026-05-27** | done | All `ParameterBlock<…>` declarations now carry `[[vk::binding(0, 0)]]`. Sweep (`TestM233RenderBindingSetRatchet::test_slang_lib_parameterblocks_use_set0_binding`) caught the last violator at `shaders/lib/bucket.slang:119` (`searchsorted_args`); fixed. 4/4 binding-set ratchets PASS — covers `templates/*.slang`, `templates/*.jinja`, `kernel/header.py`, and `shaders/lib/*.slang`. |
...
| **M-pipeline-3** | `VulkanKernel._compute_config_key` is incomplete | ✅ **DONE 2026-05-24** — `kernel/main.py:382-513` now hashes input/output buffer dtypes (`in_dt:…`, `out_dt:…`), push-constant layout (`pc:…`), inplace count (`ip:…`), and ParameterBlock flag (`pb:…`). Key prefix bumped `cfg_` → `cfg2_` to invalidate old entries. Tests: `TestMPipeline3ConfigKeyComplete` (10+ tests in `test_inductor_regression.py:49922`). |
...
| **M-pipeline-6** | `_VulkanCustomPass.uuid()` hashes only `__init__.py` | ✅ **DONE 2026-05-24** — `fx_passes/__init__.py:90-127` computes `_FX_PASSES_SUBTREE_UUID` at import time by hashing all `*.py` files in the `fx_passes/` subtree via `_compute_fx_passes_subtree_uuid()`. `_VulkanCustomPass.uuid()` (line 186) returns this bytes value — O(1) per compile, full subtree coverage. `TORCH_VULKAN_FX_PASS_UUID_DEBUG=1` prints file list + digest for debugging. |
