...
| **AOTI.1** | V13-AOTI | AOTI C++ codegen for Conv+GN models | 1 d | ⚠️ **2026-06-02 (progress).** PF.60 (tensor_str recursion) monkey-patch applied (606d7b3). PF.13 (FunctionalTensor data_ptr) → set_.source_Storage + _storage_offset registered (b9d60dc). C++ allocation codegen fixed: proper dtype/shape encoding (c76f5a5). **Next blocker:** Vulkan custom ops (aoti_torch_vulkan_mm_out) need C++ shim wrappers — not yet declared. VulkanCppWrapperGpu registered and incrementally unblocked. |
