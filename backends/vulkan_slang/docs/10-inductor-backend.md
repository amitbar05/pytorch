...
| **TRAIN.11** | mm_tile.slang precompilation | Autotune fails with `mm_tile.slang-module not found after precompilation` for all addmm choices → `best_time: Infinity` → NoValidChoicesError. Root cause: `select_algorithm.py` precompiles `shaders/lib/mm_tile.slang` but the generated `.spv` file is not found at runtime. Likely a path resolution issue between slangc output and runtime lookup. Blocks Linear layer compilation (used in SmallCNN North Star). Workaround: use 1×1 Conv2d instead of Linear (SimpleCNN already does this). | 1 d | 🔴 **OPEN** (discovered 2026-05-28). Separated from TRAIN.8 — the partitioner issue is resolved, but Linear/addmm remains blocked. |
...
| TRAIN.11 | 🔴 OPEN | — | `TestTrain11MmTilePrecompilation` (not yet written) |
