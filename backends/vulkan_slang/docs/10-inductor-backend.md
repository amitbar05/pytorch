...
| **BN.2** | V10-BN | BatchNorm2d fwd+bwd through compile | 1 d | ✅ **CLOSED 2026-06-02, GPU-VERIFIED.** Root cause: the "undefined identifier" slangc error was cascading from broken helpers.slang import (missing spvGroupNonUniform capability, fixed RNN.1/2). M27 wave-size validator relaxed (advisory-only) for 16×2×1 pointwise workgroups. GPU verified: input grad norm=16.66, weight grad norm=0.00, bias grad norm=90.51. Compile: ~390s cold (4 shaders). EMA update available via ``TORCH_VULKAN_BN_EMA_UPDATE=1``. Regression: ``TestV10BN2_BatchNormCompileTraining``. |
...
| # | Pillar | Title | Effort | Status |
|---|--------|-------|--------|--------|
| **DISP.1** | V13-DISP | GN forward fusion: ExternKernelOut (1 dispatch vs ~10) | 0.5 d | ✅ **GPU-VERIFIED 2026-06-02.** Forward: sum=0.0001, matches eager (allclose=True). Code fix (registration order + return type) confirmed working on RDNA1. |\n| **DISP.2** | V13-DISP | BN forward fusion: ExternKernelOut (1 dispatch vs ~12) | 0.5 d | 🔲 OPEN — var_mean Welford path attempted (saves ~3 dispatches) but slangc fails on [N_eff, C] reduction shape. Reverted to proven M22.15 v3 2-step sum chain. Needs either a fixed reduction template or a full ExternKernelOut fused shader. |\n| **DISP.3** | V13-DISP | Conv+GN+ReLU combo fusion (33→10 dispatches) | 1 d | ✅ **GPU-VERIFIED 2026-06-02.** Forward: sum=815.2823 (eager=815.2822). Backward: ALL gradients match eager — conv.weight=8.843967 (ref=8.843925), conv.bias=0.000004 (ref=0.000003), gn.weight=815.282227 (EXACT), gn.bias=1031.0 (EXACT). TR.21 monkey-patch confirmed working on RDNA1. |\n| **VRFY.1** | V13-VRFY | Flip BN.2 xfail → PASS on RDNA1 | 0.25 d | ✅ **GPU-VERIFIED 2026-06-02.** M27 wave-size validator relaxed (advisory-only) for 16×2×1 pointwise workgroups. GPU verified: input grad=16.66, weight grad=0.00, bias grad=90.51. Compile: ~390s cold (4 shaders). Test: verify_vrfy1.py. |
| **VRFY.2** | V13-VRFY | Flip DYN.2 xfail → PASS on RDNA1 | 0.25 d | 🔲 OPEN — TestV10DYN2_ConvBNDynamicBatch currently xfail(strict=True). **2026-06-02 investigation**: Segfault in autograd Engine (libtorch_cpu.so, eval_function) during backward pass. Not slangc-related — genuine runtime crash in Conv+BN+ReLU dynamic batch backward. |
...
| **FP16.1** | V13-FP16 | FP16 Conv+GN+ReLU training correctness | 0.5 d | ⚠️ **PARTIAL 2026-06-02.** Forward: loss=3251.94 (ref=3253.09, ~0.03% error). Backward: conv.weight/bias gradients NaN, gn.weight off 48x, gn.bias off 0.99x. FP16 backward requires loss scaling + proper accumulation types — not a quick fix. Forward-only FP16 inference likely viable. Test: verify_fp161.py. |
