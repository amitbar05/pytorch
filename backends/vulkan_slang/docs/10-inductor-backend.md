...
| **M18.TB.1** | `threshold_backward` inline lowering 3-arg/2-arg signature mismatch | ❌ **NEW 2026-05-23** | `_make_inline_unary_bwd_diff_lowering` registers `_lowering(grad_output, self)` for `aten.threshold_backward` but the aten op schema is `(Tensor grad_output, Tensor self, Scalar threshold) → Tensor`. The mismatched call silently falls through, producing an empty-tensor gradient. `HardtanhBackward0` then fails with "got [] but expected shape compatible with [32]". Fix: extend the inline lowering signature to `(grad_output, self, threshold)` and ignore `threshold` (the Slang `relu_fwd` always uses threshold=0). Affects `F.hardtanh` backward. See `test_hardtanh_backward_grad_parity` (xfail strict=False). |
...
| **TEST.COV.3** | ✅ **DONE 2026-05-23** — `TestTestCov3ActivationBwd` (24 tests) added for hardtanh/hardsigmoid/softplus/mish + 6 basic trig (sin/cos/tan/asin/acos/atan) + 6 hyperbolic (sinh/cosh/tanh/asinh/acosh/atanh) + 8 exp/log (exp2/expm1/log2/log10/log1p/erf/erfc/sqrt). All dispatch through `bwd_diff_table.py` → `bwd_diff(<op>_fwd)` → pointwise backward shaders, except `softplus` (algebraic route via M-AG5.1) and `hardtanh` (via threshold_backward). Note: `torch.special.cbrt` not available in PyTorch 2.11; excluded. **2 tests xfail (strict=False):** `test_hardtanh` (M18.TB.1: threshold_backward 3-arg/2-arg signature mismatch); `test_hardsigmoid` (M18.HS.1: hardsigmoid decomposes to clamp → FakeTensorProp data-access error). 22 remaining tests require warm SPIR-V cache (`@pytest.mark.slow_compile(seconds=600)`) due to FA prewarm thread-pool blocking on cold cache. |
