...
| **TRAIN.8** | Conv training correctness sweep | End-to-end correctness validation for 3 conv model classes under torch.compile: (A) Conv+BN+FC (ResNet-mini), (B) Conv+GN+ReLU+FC (SmallCNN), (C) Conv+MaxPool+FC (SimpleCNN). Each must match CPU loss curve within rtol=0.5 over 10 training steps. | 0.5 d | 🚫 **BLOCKED — AOT partitioner** (2026-05-27). Tests written and xfail-marked. Root cause: AOT min-cut partitioner marks forward nodes that depend on `target` (integer class indices used in cross_entropy) as InvalidNode. Investigated: custom nll_loss_forward decomp, removing nll_loss_forward from decomp tables, manual cross_entropy (log_softmax+gather+neg+sum). All fail with same error. Likely cause: Dynamo graph break at `loss.backward()` causes `target` to enter the compiled sub-graph as closure variable rather than primal input. Needs interactive debugging session. |
