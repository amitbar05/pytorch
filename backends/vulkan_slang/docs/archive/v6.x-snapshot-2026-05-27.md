# v6.x reference appendix — snapshot 2026-05-27

This file is a frozen snapshot of the pre-v7 sections of
[`docs/10-inductor-backend.md`](../10-inductor-backend.md) taken on
2026-05-27 when the v7 plan replaced v6.3/v6.5 as the active roadmap.

It is retained for back-link integrity and to preserve the decision
record around M9, M11, M12, M14–M23, M-NEW.*, M-pipeline.*,
TEST.COV.*, M-docs.*, M-CPP-AUDIT, EAGER.*, and the five-agent /
Wave 3 audits (§ 0.5 through § 0.7). Each section is dated and
self-describing.

**Do not extend this file.** New work goes under § v7 in
[`docs/10-inductor-backend.md`](../10-inductor-backend.md).
