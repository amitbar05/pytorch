...
| **MS.2** | **Use-after-free: AOTI shim `zeros/ones/full/as_strided` return dangling `AtenTensorHandle`** (`csrc/backend/aoti_shims.cpp:146,172,193,215`): each function returns `tensor.unsafeGetTensorImpl()` of a local `at::Tensor` that is destroyed on return — every AOTI model using zeros/ones init crashes or corrupts silently. Fix: allocate with `new at::Tensor(std::move(tensor))` at all four sites (matching `empty_strided_vulkan` at line 98). | 🔴 **OPEN** — critical use-after-free |
| **MS.1** | **Memory leak: `aoti_torch_delete` is a no-op** (`csrc/backend/aoti_shims.cpp:154-156`): `RAIIAtenTensorHandle` destructor calls this for every intermediate tensor; the handle is silently discarded. Leaks every `new at::Tensor` allocated by the shim layer. Fix: `delete reinterpret_cast<at::Tensor*>(handle)`. **Must land with MS.2.** | 🔴 **OPEN** — memory leak on every AOTI dispatch |
